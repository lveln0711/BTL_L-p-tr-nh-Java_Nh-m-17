...
public class CustomerView {
    ...
    private void buildUI() {
        ...
        idCol.setCellValueFactory(d -> new SimpleStringProperty(d.getValue().getCustomerId()));
        idCol.setPrefWidth(90);

        TableColumn<Customer, String> nameCol = new TableColumn<>("Tên khách hàng");
        nameCol.setCellValueFactory(d -> new SimpleStringProperty(d.getValue().getCustomerName()));
        ...
        tfSearch.textProperty().addListener((obs, oldVal, newVal) -> {
            if (newVal == null || newVal.trim().isEmpty()) {
                ...
                List<Customer> result = AppContext.CUSTOMER_MANAGER.searchCustomersByName(newVal);
        ...
        btnDelete.setOnAction(e -> {
            ...
            boolean confirm = AlertHelper.showConfirm("Xác nhận xóa","Bạn có chắc muốn xóa khách hàng: " + selected.getCustomerName() + "?");
            if (confirm) {
                boolean ok = AppContext.CUSTOMER_MANAGER.deleteCustomerById(selected.getCustomerId());
                if (ok) {
    ...
    private void showAddDialog() {
        ...
        dialog.setResultConverter(btn -> {
            ...
            return buildCustomerFromForm(tfId, tfName, tfPhone, tfAddress);
        });
    ...
    private Customer buildCustomerFromForm(TextField tfId, TextField tfName, TextField tfPhone, TextField tfAddress) {
        try {
