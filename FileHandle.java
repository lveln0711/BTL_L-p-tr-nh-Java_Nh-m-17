...
public class FileHandler {
    ...
    private static String productToData(Product product) {
        if (product instanceof Phone phone) {
            return "PHONE," + phone.getProductId() + "," + phone.getProductName() + "," + phone.getBrand() + ","  + phone.getProductPrice() + ","  + phone.getQuantity() + ","  + phone.getRam() + ","  + phone.getStorage();
        }
        if (product instanceof Laptop laptop) {
            return "LAPTOP," + laptop.getProductId() + ","  + laptop.getProductName() + ","  + laptop.getBrand() + ","  + laptop.getProductPrice() + ","  + laptop.getQuantity() + ","  + laptop.getCpu() + ","  + laptop.getRam();
        }
        if (product instanceof TV tv) {
            return "TV," + tv.getProductId() + ","  + tv.getProductName() + ","  + tv.getBrand() + ","  + tv.getProductPrice() + ","  + tv.getQuantity() + ","  + tv.getSize() + ","  + tv.getResolution();
        }
        return "";
    }
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(file))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                try {
                    ...
                    Customer customer = new Customer(parts[0].trim(), parts[1].trim(),parts[2].trim(), parts[3].trim());
     ...
     */
    public static List<Invoice> loadInvoicesFromFile(String fileName,List<Customer> customerList,List<Product> productList) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                try {
                    ...
                    for (String itemData : itemParts) {
                        ...
                        InvoiceItem item = InvoiceItem.loadFromFile(product, quantity, unitPrice);
                        invoice.getItemList().add(item);
                    }

                    if (!invoice.getItemList().isEmpty() && !containsInvoiceId(invoiceList, invoice.getInvoiceId())) {
