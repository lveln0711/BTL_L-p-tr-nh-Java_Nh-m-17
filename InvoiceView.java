...
public class InvoiceView {
    ...
    private void buildUI() {
        ...
        idCol.setCellValueFactory(d -> new SimpleStringProperty(d.getValue().getInvoiceId()));
        ...
        itemsCol.setCellValueFactory(d -> new SimpleStringProperty(String.valueOf(d.getValue().getItemList().size())));
        itemsCol.setPrefWidth(60);

        TableColumn<Invoice, String> totalCol = new TableColumn<>("Tổng tiền (VNĐ)");
        totalCol.setCellValueFactory(d -> new SimpleStringProperty(String.format("%,.0f", d.getValue().getTotalAmount())));
        totalCol.setPrefWidth(140);

        table.getColumns().setAll(List.of(idCol, customerCol, dateCol, itemsCol, totalCol));
        ...
        btnDelete.setOnAction(e -> {
            ...
            if (confirm) {
                ...
                boolean ok = AppContext.INVOICE_MANAGER.deleteInvoiceById(selected.getInvoiceId());
                if (ok) {
                    // Lưu lại tồn kho đã hoàn vào file
                    AppContext.PRODUCT_MANAGER.saveToFile();
                    AlertHelper.showInformation("Thành công", "Đã xóa hóa đơn và hoàn kho tự động!");
                    refresh();
                } else {
    ...
    private void showCreateInvoiceDialog() {
        ...
        if (customers.isEmpty()) {
            AlertHelper.showWarning("Cảnh báo","Chưa có khách hàng nào! Vui lòng thêm khách hàng trước.");
            return;
        }
        if (products.isEmpty()) {
        ...
        cbCustomer.setCellFactory(lv -> new ListCell<>() {
            ...
            protected void updateItem(Customer c, boolean empty) {
                ...
                setText(empty || c == null ? null : c.getCustomerId() + " - " + c.getCustomerName());
            }
        });
        ...
        cbProduct.getItems().addAll(products.stream().filter(p -> p.getQuantity() > 0).toList());
        cbProduct.setPromptText("-- Chọn sản phẩm --");
        cbProduct.setCellFactory(lv -> new ListCell<>() {
            @Override
            protected void updateItem(Product p, boolean empty) {
                super.updateItem(p, empty);
                setText(empty || p == null ? null : p.getProductId() + " - " + p.getProductName() + " (Tồn: " + p.getQuantity() + ")");
            }
        });
        ...
        colName.setCellValueFactory(d -> new SimpleStringProperty(d.getValue().getProduct().getProductName()));
        ...
        colSub.setCellValueFactory(d -> new SimpleStringProperty(String.format("%,.0f", d.getValue().getSubTotal())));
        ...
        Runnable recalcTotal = () -> {
            double total = itemData.stream().mapToDouble(InvoiceItem::getSubTotal).sum();
            ...
            for (InvoiceItem item : itemData) {
                if (item.getProduct().getProductId().equals(selectedProduct.getProductId())) {
                    ...
                    if (newQty > selectedProduct.getQuantity()) {
                        AlertHelper.showError("Lỗi", "Vượt quá tồn kho! Còn: " + selectedProduct.getQuantity());
                        return;
                    }
        ...
        dialog.setResultConverter(btn -> {
            ...
            if (itemData.isEmpty()) {
                AlertHelper.showWarning("Cảnh báo", "Hóa đơn phải có ít nhất 1 sản phẩm!");
                return null;
            }
        ...
        result.ifPresent(invoice -> {
            try {
                ...
                if (ok) {
                    ...
                    AppContext.PRODUCT_MANAGER.saveToFile();
                    AlertHelper.showInformation("Thành công", "Đã tạo hóa đơn " + invoice.getInvoiceId() + "!");
                    refresh();
                } else {
    ...
    private void showInvoiceDetail(Invoice invoice) {
        ...
        sb.append("Khách hàng : ").append(invoice.getCustomer().getCustomerName()).append("\n");
        sb.append("Ngày       : ").append(invoice.getInvoiceDate()).append("\n");
        sb.append("─────────────────────────────────\n");

        for (InvoiceItem item : invoice.getItemList()) {
            sb.append(String.format("%-25s x%d  %,.0f VNĐ%n",item.getProduct().getProductName(),item.getQuantity(),item.getSubTotal()));
        }
        sb.append("─────────────────────────────────\n");
